...
export async function proxy(request: NextRequest) {
  ...
  if (cspFetchEnabled) {
    try {
      ...
      if (!iframeOrigins) responseHeaders.set("X-Frame-Options", "deny");
    } catch (err) {
  ...
  if (publicHost) requestHeaders.set("x-zitadel-public-host", publicHost);
  if (instanceHost) requestHeaders.set("x-zitadel-instance-host", instanceHost);
