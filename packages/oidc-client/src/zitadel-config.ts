...
export function createZitadelConfig(input: ZitadelConfigInput): UserManagerSettings {
  return {
    ...
    response_type: "code",
    scope: input.scope ?? "openid profile email offline_access urn:zitadel:iam:org:project:roles urn:zitadel:iam:org:project:id:zitadel:aud",
    metadata: {
      ...
      jwks_uri: `${input.authority}/oauth/v2/keys`,
