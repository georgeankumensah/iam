...
iam_project = [p for p in projects if p["name"] == "IAM 3.0"]
if not iam_project:
    print("IAM 3.0 project not found!")
    sys.exit(1)
pid = iam_project[0]["id"]
print(f"IAM 3.0 project id: {pid}")
...
for a in apps:
    ...
    print(f"  {name}: clientId={cid}, state={a.get('state', '?')}")
