...
export default function AdminUsersPage() {
  ...
  const load = useCallback(async (p: number, q: string) => {
    ...
    try {
      ...
      const resp = await fetch(`/api/admin/users?${params}`, { cache: "no-store" });
      if (!resp.ok) throw new Error("Sign in as a superadmin to manage users");
  ...
  async function handleReactivate(user: UserRecord) {
    ...
    try {
      const resp = await fetch(`/api/admin/users/${user.id}/reactivate`, { method: "POST" });
      const data = await resp.json();
      if (!resp.ok || !data.success) {
        throw new Error(data.error || "Could not reactivate user");
      }
      await load(page, search);
    } catch (err) {
          ...
          <div className="mb-5 flex flex-wrap items-start justify-between gap-4">
            <div>
              <h2 className="text-xl font-semibold">
                <Users size={20} className="mr-2 inline-block align-text-top" />
                Users
              </h2>
                ...
                <tbody>
                  {users.map((user) => (
                    <tr key={user.id} className="border-b border-[#f4f6f2] hover:bg-[#f9faf7]">
                      <td className="py-3 pr-4 font-medium">{userLabel(user)}</td>
